...
#elif defined(osf5) && defined(RET_PROC_CMDLINE_FULL)
...
void get_argv(int fd, off_t argv,int size, char **buf) {
	int i;
	char * ptr, * buf2;
	int was_zero = 0;

	ptr = buf2 = malloc(size);

	if (pread(fd,buf2,size,argv) < 0)
		perror("pread");
...
sh_count_procs(char *procname)
{
	DIR * dir;
	struct dirent * ent;
	char le_file[100], *tempc;
    int plen = strlen(procname), total = 0, matchResultCode = 0, matchResultError = 0;
	regex_t regexBuffer, myMatch;
	...
	while ((ent = readdir(dir)) != NULL)
	{
		...
		sprintf(le_file,"%s/%s","/proc",ent->d_name);
		
		tempc = malloc(sizeof(char) * 1000);
		
		get_info(le_file, &tempc);
		
		DEBUGMSGTL(("proc","Comparing wanted %s against %s\n", procname, tempc));
		
		matchResultError = regcomp(&(regexBuffer), procname, REG_EXTENDED | REG_NOSUB);
		
		if (matchResultError == 0)
		{
			matchResultCode = regexec(&(regexBuffer), tempc, 0, &myMatch, REG_NOTEOL);
		}
		else
		{
			matchResultCode = 1;
            DEBUGMSGTL(("proc", " Could not create RegEx\n", total));
		}
...
/*		if(!strncmp(tempc, procname, plen)) {
            total++;
            DEBUGMSGTL(("proc", " Matched.  total count now=%d\n", total));
		}
*/	  
...
sh_count_procs(char *procname)
{
    ...
    for (;;) {
      ...
      DEBUGMSGTL(("proc","ultrix Comparing wanted %s against %s\n",
                  procname, auser->u_comm));
...
sh_count_procs(char *procname)
{
    ...
    while ((ent = readdir(dir))) {
        ...
        if (!info.pr_nlwp && !info.pr_lwp.pr_lwpid) {
            ...
            DEBUGMSGTL(("proc","Solaris Comparing wanted %s against %s\n",
